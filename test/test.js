 ...
 */
function makePrinterEventHandler(delay = 2) {
  return {
    printEvent: ({
      type,
      newPosition,
    ...
    gcodeEvent: () => new Promise((resolve) => setTimeout(resolve, delay)), // wait 20 ms
  };
}
...
infoMsg(`STARTING: ${new Date() * 0.01}`);
...
async function setup() {
  ...
  liveprintercomms.schedule(async () =>
    infoMsg(`schedule test: ${testIndex++}`)
  );
...
async function startTest() {
  ...
  await liveprintercomms.schedule(async () =>
    printer.moveto({ x: printer.cx, y: printer.cy })
  );
  ...
  ASSERT(
    printer.x,
    origX + distToMove * Math.cos(printer.d2r(25)),
    `did not move ${distToMove}`
  ); // off by a few hundredths... not great
  ASSERT(
    printer.y,
    origY + distToMove * Math.sin(printer.d2r(25)),
    "y not equal to y0"
  );
  ...
  printer.warp = ({ d, heading, elevation, t, tt } = {}) => {
    ...
    const h2 = heading / 2;
    Logger.debug(
      `warp(d:${d}, heading:${heading}, elevation:${elevation}, t:${t}, tt:${tt})`
    );
  ...
  infoMsg(`One beat at 140bpm should be ${60000 / 140} ms`);
  printer.interval("1/8b");
  ASSERT(
    printer._intervalTime,
    (60000 / 140) * 0.125,
    "Interval time miscalculation",
    `TInterval: One beat at 140bpm should be ${60000 / 140} ms`
  );
  ...
  infoMsg(`--- x at ${printer.x} / ${origX}`);

  await liveprintercomms.schedule(async () =>
    printer.to({ x: printer.x + 4, y: printer.y, z: printer.z })
  );
  await liveprintercomms.schedule(async () => printer.draw());

  ASSERT(printer.x, origX + 4, `did not move ${4}`); // off by a few hundredths... not great
  ASSERT(printer.y, origY, "y not equal to y0");
  ASSERT(printer.z, origZ, "z not equal to z0");
  ASSERT(origE, printer.e, "e SHOULD be different");
...
async function startMusicTest() {
  ...
  await liveprintercomms.globalEval(
    'lib.log.info(`A4/3P:${ lib.transpose("A4","3M") }`)'
  );
