// may 12 2023 -- first attempt livecoding actual printer funcs with inject
...
lp.warp = function ({d, heading, elevation, t, tt}={})
{
  const bt = lp.b2t(1/2);
  const tt2 = tt % (32*bt);
  heading = heading + Math.PI/6*Math.sin(Math.PI*tt2/bt) + Math.PI/8*Math.sin(Math.PI*(tt2)/bt); // sine wave for 1/32 beat
...
const W = Math.ceil(((lp.maxx*0.4)-2*pad) / (Math.max(ROWS,COLS)+1));
